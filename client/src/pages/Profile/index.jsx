...
export default function Profile() {
    ...
    useEffect(() => {
        ...
        const fetchProfileData = async () => {
            try {
                ...
                setCat(Array.isArray(catData) && catData.length > 0 ? catData[0] : null);
                setCompletedSessions(totalCompleted);
                setCoins(Number(userData.coins || 0));
                setHoursFocused(Number(totalHours.toFixed(1)));
            } catch (err) {
                setPageError(err.message || "Something went wrong while loading your profile.");
            } finally {
                setPageLoading(false);
            }
        };
    ...
    const handleDeleteAccount = async () => {
        ...
        try {
            ...
            setPageError(err.message || "Unable to delete account right now.");
        } finally {
            setIsDeleting(false);
    ...
    const handleConfirmSignOut = async () => {
        ...
        try {
            ...
            setPageError(err.message || "Unable to sign out right now.");
        } finally {
    ...
    const handleOpenPetEditor = () => {
        ...
        const matchedCatOption =
            cats.find((option) => option.img === cat.image || option.imgSelected === cat.image) || null;
    ...
    return (
        ...
        >
                    ...
                    >
                        <CardContent sx={{ p: 3 }}>
                            ...
                            >
                                ...
                                >
                                    <Avatar sx={{ mx: "auto", mb: 1, bgcolor: "#fff3f2", color: "#ef8f8b" }}>
                                        <EmojiEventsOutlinedIcon />
                                    </Avatar>
                                    <Typography variant="h4">{completedSessions}</Typography>
                                    <Typography color="text.secondary">Sessions completed</Typography>
                                </Box>
                                <Box
                                    sx={{
                                        borderRadius: 2,
                                ...
                                >
                                    <Avatar sx={{ mx: "auto", mb: 1, bgcolor: "#fff8df", color: "#ccb35a" }}>
                                ...
                                >
                                    <Avatar sx={{ mx: "auto", mb: 1, bgcolor: "#ecf5fa", color: "#6a9cc0" }}>
                            ...
                            {cat ? (
                                ...
                                >
                                    ...
                                    <Stack spacing={0.75} alignItems={{ xs: "center", sm: "flex-start" }}>
                                        ...
                                        <Typography color="text.secondary">Energy: {cat.energy}</Typography>
                                    </Stack>
                                </Box>
                            ) : (
                                <Stack spacing={1.5} alignItems="center">
            ...
            <Dialog open={petEditOpen} onClose={handleClosePetEditor} maxWidth="sm" fullWidth>
                ...
                <DialogContent>
                    <Stack spacing={2} sx={{ mt: 1 }}>
                                ...
                                {cats.map((catOption) => (
                                    <ToggleButton key={catOption.type} value={catOption}>
                                        <Stack spacing={0.5} alignItems="center">
                                            ...
                                            />
                                            <Typography variant="caption">{catOption.type}</Typography>
                                        </Stack>
                                    </ToggleButton>
                                ))}
                            </ToggleButtonGroup>
                        ...
                        {petEditError && <Typography color="error">{petEditError}</Typography>}
                    </Stack>
                </DialogContent>
                <DialogActions sx={{ px: 3, pb: 2 }}>
                    <Button onClick={handleClosePetEditor} disabled={petSaving}>Cancel</Button>
                    <Button
                        variant="contained"
                        onClick={handleSavePetChanges}
                        disabled={petSaving || !trimmedPetName || !selectedPetImage || !hasPetChanges}
                    >
